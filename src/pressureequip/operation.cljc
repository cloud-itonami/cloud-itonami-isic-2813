(ns pressureequip.operation
  "OperationActor -- one pressure-equipment-manufacturing operation =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (Pressure Equipment Advisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Pressure
  ...
  One graph run = one pressure-equipment-manufacturing operation
  ...
  decision to a human operator (the pressure-equipment manufacturing
  ...
  (:require [langgraph.graph :as g]
            ...
            [pressureequip.governor :as governor]
...
(defn build
  ...
  `pressureequip.store/Store`).
  opts:
    :advisor      -- a `pressureequip.pressureequipadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (pressureequipadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Pressure Equipment Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (pressureequipadvisor/-advise advisor store request)]
            {:proposal p :audit [(pressureequipadvisor/trace request p)]})))

      ;; Pressure Equipment Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
